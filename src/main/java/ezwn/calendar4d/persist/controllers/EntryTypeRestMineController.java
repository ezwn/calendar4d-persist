...
@RestController
@RequestMapping(value="/entry-type/mine", produces = MediaType.APPLICATION_JSON_VALUE)
public class EntryTypeRestMineController {
   
   private EntryTypeService entryTypeService;
   private EntitiesDTOsMapper entitiesDTOsMapper;
   
   public EntryTypeRestMineController(final EntitiesDTOsMapper entitiesDTOsMapper, final EntryTypeService entryTypeService) {
   ...
   public Iterable<EntryTypeDTO> findAll() {
      return StreamSupport.stream(entryTypeService.findAllByCalendarUser().spliterator(), false).map(i -> entitiesDTOsMapper.toDTO(i)).collect(Collectors.toList());
   }
